...
{#if isExternal}
	<a
		...
		class="{baseStyles} {underline ? underlineStyles : ''} {showIcon
	...
	>
		...
		{#if showIcon}
			<span
				class="inline-flex w-0 overflow-hidden opacity-0 transition-all duration-200 group-hover/link:ml-1 group-hover/link:w-3 group-hover/link:opacity-100"
			>
...
{:else}
	<a {href} class="{baseStyles} {underline ? underlineStyles : ''} {className}">
		{@render children()}
	</a>
{/if}
